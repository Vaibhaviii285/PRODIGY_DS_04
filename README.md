# PRODIGY_DS_04

Sentiment Analysis on Social Media Data A simple project to analyze and visualize public sentiment (Positive / Neutral / Negative) from social media text data using Python. 

* Dataset Use this Kaggle dataset:
🔗 Twitter US Airline Sentiment https://www.kaggle.com/datasets/crowdflower/twitter-airline-sentiment 

* Features
1.Data Cleaning & Preprocessing
2.Tokenization
3.Removing Stopwords
4.Sentiment Classification using VADER
5.Visualizations (Bar chart, Pie chart, Wordcloud)
6.Exporting cleaned + analyzed dataset
